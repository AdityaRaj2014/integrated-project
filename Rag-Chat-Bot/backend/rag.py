...
class RAGEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"[WARN] Embedding model unavailable, using keyword fallback retrieval. Error: {exc}")
    ...
    def _build_hospital_index(self, hospital_id: str):
        ...
        print(f"[RAG] Building index for hospital: {hospital_id}")
        ...
        if diseases_dir.exists():
            for disease_file in diseases_dir.glob("*.txt"):
                ...
                print(f"  [OK] Processed disease: {disease_name} ({len(chunks)} chunks)")
        ...
        if templates_dir.exists():
            for template_file in templates_dir.glob("*.txt"):
                ...
                all_chunks.extend(chunks)
                print(f"  [OK] Processed template: {template_file.name} ({len(chunks)} chunks)")
        ...
        if guidelines_dir.exists():
            for guideline_file in guidelines_dir.glob("*.txt"):
                ...
                all_chunks.extend(chunks)
                print(f"  [OK] Processed guideline: {guideline_file.name} ({len(chunks)} chunks)")
        ...
        if wellness_dir.exists():
            for wellness_file in wellness_dir.glob("*.txt"):
                ...
                all_chunks.extend(chunks)
                print(f"  [OK] Processed wellness: {wellness_file.name} ({len(chunks)} chunks)")
        ...
        if disclaimers_file.exists():
            ...
            all_chunks.extend(chunks)
            print(f"  [OK] Processed disclaimers ({len(chunks)} chunks)")
        
        if not all_chunks:
            print(f"  [WARN] No data found for {hospital_id}")
            return
        
        # Store chunk metadata in memory for both vector and fallback modes.
        ...
        print(f"  [OK] Index built: {len(all_chunks)} total chunks")
    ...
    def _load_existing_index(
        ...
        try:
            print(f"[RAG] Loading existing index for {hospital_id}")
            ...
            print(f"  [OK] Loaded {len(self.chunks[hospital_id])} chunks")
            return True
    ...
    def retrieve(
        ...
        index = self.indexes.get(hospital_id)
        ...
        filtered_results = []
        if self.encoder is not None and index is not None:
            # Encode query
            ...
            # Offline keyword fallback ranking when embeddings are unavailable.
            query_terms = [t for t in re.findall(r"[a-z0-9]+", query.lower()) if len(t) > 2]
            scored: List[Tuple[float, int]] = []
            for idx in filtered_indices:
                text = all_chunks[idx].text.lower()
                score = 0.0
